...
//        x.foreach(println)
    ...
    it should "contain only correct output" in {
        ...
        //          .map(_.toDense)
        //        v.foreach(println)
        d.run(sc, vecs, 5, 0.0, outputDirectory = outputDirectory).collect()
        val answer = sc.textFile(outputDirectory+"/*").map(s => s.split(",")).map(a => ((a(0).toLong, a(1).toLong),a(2).toDouble) ).collect().sorted
        println(s"count: ${answer.size}")
        answer.foreach{
            case(i,j) =>
                //                println(s"for pair $i, expected: ${expected(i)} got: $j")
