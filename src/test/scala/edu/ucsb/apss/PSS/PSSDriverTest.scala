...
//        x.foreach(println)
    ...
    ignore should "b" in {
        ...
        val converter = new TextToVectorConverter
        val vecs =   converter.convertTweetsToVectors(par, maxWeight = 20, removeSWords = true, topToRemove = 4)

//        val vecs =   par.map(converter.convertTweetToVector(_,maxWeight = 5, removeSWords = true, topToRemove = 4))
        val executionValues = List(.9)
        val buckets = 21
        val theoreticalStaticPartitioningValues = ArrayBuffer[Long]()
        val actualStaticPartitioningValues = ArrayBuffer[Long]()
