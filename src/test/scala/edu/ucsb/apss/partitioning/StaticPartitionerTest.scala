...
//
    "tieVectorsToHighestBuckets" should "take every vector and tie it to the bucket which has the closest but < leader to its lInf" in {
        val rdd = sc.parallelize(Source.fromInputStream(getClass.getResourceAsStream("10-tweets-bag.txt")).getLines().toList).map((new TextToVectorConverter).convertTextToVector(_))
