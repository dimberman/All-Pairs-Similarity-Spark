...
case class InvertedIndexCalcuation(partition:Int, bucket:Int, tl:Int, x:Seq[Int], y:Seq[Long], z:Seq[Double])
  ...
  private[apss] def calculateCosineSimilarityByPullingFromFile(invertedIndexes: RDD[((Int, Int), Iterable[SimpleInvertedIndex])], threshold: Double, numBuckets: Int, balancedMapping: Map[(Int, Int), List[(Int, Int)]], calcSize: Int = 100): RDD[(Long, Long, Double)] = {
    ...
    val balancedInvertedIndexes = invertedIndexes.flatMap {
      case (bucket, inv) =>
        inv.map {
          i =>
              val (x,y,z) = i.toDatasetCompatible()
            InvertedIndexCalcuation(partMap(bucket), bucket._1, bucket._2, x.toArray, y.toArray, z.toArray)
        }
    ...
    val invertedIndexDataset = sqlContext.createDataset(balancedInvertedIndexes)
