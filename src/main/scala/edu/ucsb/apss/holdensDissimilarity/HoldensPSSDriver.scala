...
class HoldensPSSDriver() {
    ...
    def run(sc: SparkContext, vectors: RDD[SparseVector], numBuckets: Int, threshold: Double) = {
        ...
        breakdown.foreach{case(k,v) => skipped += k._2*bucketSize}
        ...
        println()
        println("breakdown:")
        breakdown.toList.sortBy(_._1).foreach(println)
        println()
        println("***************************************")
        println()
    ...
    def calculateCosineSimilarityUsingCogroupAndFlatmap(partitionedTasks: RDD[(Int, (Iterable[(Int, VectorWithNorms)], Iterable[InvertedIndex]))], threshold: Double, numBuckets: Int): RDD[(Long, Long, Double)] = {
        ...
        val similarities: RDD[List[Similarity]] = partitionedTasks.mapPartitions {
            iter =>
                iter.map {
                    case (idx, (vectors, i)) =>
                        calculateSimilarityForPartition(idx, vectors, i, threshold)
                }
        }
    ...
    def calculateSimilarityForPartition(idx: Int, vectors: Iterable[(Int, VectorWithNorms)], i: Iterable[InvertedIndex], threshold:Double): List[Similarity] = {
